<template>
    <div>
    <div class="promo center">
        <section class="promo__content">
            <h2 class="promo__h2 line__hight_drand">THE BRAND</h2>
            <h3 class="promo__h3">OF LUXERIOUS <span class="promo__color">FASHION</span></h3>
        </section>
    </div>
    <div class="blocks_img center">
        <div class="block__img1 block__img_trans">
            <article class="text__block1 block__text_transform">
                <p class="title_block_img">HOT DEAL</p>
                <h3 class="title_block_img2">FOR MEN</h3>
            </article>
        </div>
        <div class="block__img3 block__img_trans">
            <article class="text__block3 block__text_transform">
                <p class="title_block_img">LUXIROS & TRENDY</p>
                <h3 class="title_block_img2">ACCESORIES</h3>
            </article>
        </div>
        <div class="block__img2 block__img_trans">
            <article class="text__block2 block__text_transform">
                <p class="title_block_img">30% OFFER</p>
                <h3 class="title_block_img2">WOMEN</h3>
            </article>
        </div>
        <div class="block__img4 block__img_trans">
            <article class="text__block4 block__text_transform">
                <p class="title_block_img">NEW ARRIVALS</p>
                <h3 class="title_block_img2">FOR KIDS</h3>
            </article>
        </div>
    </div>
    <div class="clr"></div>
    <section class="fetured center">
        <h3 class="fetured__title">Fetured Items</h3>
        <p class="fetured__text">Shop for items based on what we featured in this week</p>
        <products ref="products"></products>
        <error ref="error"></error>
        <div class="blocks__position_featured center">
            <div class="featured_link">
                <div class="block__featured">
                    <div class="block__self1"><img src="../assets/img/photo1.jpg" alt="photo___1"
                                                   class="photo___1 featured__photo">
                        <a href="single_page.html" class="cart__hover"> <img src="../assets/img/cart_hover.svg"
                                                                             alt="cart_hover_img"
                                                                             class="cart_hover_img">
                            <p class="hover_text">Add to Cart</p>
                        </a>
                    </div>
                    <p class="featured_text">Mango People T-shirt</p>
                    <p class="featured_text2">$52.00</p>
                </div>
            </div>
            <div class="featured_link">
                <div class="block__featured">
                    <div class="block__self2"><img src="../assets/img/photo2.jpg" alt="photo___2"
                                                   class="photo___2 featured__photo">
                        <a href="single_page.html" class="cart__hover"> <img src="../assets/img/cart_hover.svg"
                                                                             alt="cart_hover_img"
                                                                             class="cart_hover_img">
                            <p class="hover_text">Add to Cart</p>
                        </a>
                    </div>
                    <p class="featured_text">Mango People T-shirt</p>
                    <p class="featured_text2">$52.00</p>
                </div>
            </div>
            <div class="featured_link">
                <div class="block__featured">
                    <div class="block__self3"><img src="../assets/img/photo3.jpg" alt="photo___3"
                                                   class="photo___3 featured__photo">
                        <a href="single_page.html" class="cart__hover"> <img src="../assets/img/cart_hover.svg"
                                                                             alt="cart_hover_img"
                                                                             class="cart_hover_img">
                            <p class="hover_text">Add to Cart</p>
                        </a>
                    </div>
                    <p class="featured_text">Mango People T-shirt</p>
                    <p class="featured_text2">$52.00</p>
                </div>
            </div>
            <div class="featured_link">
                <div class="block__featured">
                    <div class="block__self4"><img src="../assets/img/photo4.jpg" alt="photo___4"
                                                   class="photo___4 featured__photo">
                        <a href="single_page.html" class="cart__hover"> <img src="../assets/img/cart_hover.svg"
                                                                             alt="cart_hover_img"
                                                                             class="cart_hover_img">
                            <p class="hover_text">Add to Cart</p>
                        </a>
                    </div>
                    <p class="featured_text">Mango People T-shirt</p>
                    <p class="featured_text2">$52.00</p>
                </div>
            </div>
            <div class="featured_link">
                <div class="block__featured">
                    <div class="block__self5"><img src="../assets/img/photo5.jpg" alt="photo___5"
                                                   class="photo___5 featured__photo">
                        <a href="single_page.html" class="cart__hover"> <img src="../assets/img/cart_hover.svg"
                                                                             alt="cart_hover_img"
                                                                             class="cart_hover_img">
                            <p class="hover_text">Add to Cart</p>
                        </a>
                    </div>
                    <p class="featured_text">Mango People T-shirt</p>
                    <p class="featured_text2">$52.00</p>
                </div>
            </div>
            <div class="featured_link">
                <div class="block__featured">
                    <div class="block__self6"><img src="../assets/img/photo6.jpg" alt="photo___6"
                                                   class="photo___6 featured__photo">
                        <a href="single_page.html" class="cart__hover"> <img src="../assets/img/cart_hover.svg"
                                                                             alt="cart_hover_img"
                                                                             class="cart_hover_img">
                            <p class="hover_text">Add to Cart</p>
                        </a>
                    </div>
                    <p class="featured_text">Mango People T-shirt</p>
                    <p class="featured_text2">$52.00</p>
                </div>
            </div>
            <div class="featured_link">
                <div class="block__featured">
                    <div class="block__self7"><img src="../assets/img/photo7.jpg" alt="photo___7"
                                                   class="photo___7 featured__photo">
                        <a href="single_page.html" class="cart__hover"> <img src="../assets/img/cart_hover.svg"
                                                                             alt="cart_hover_img"
                                                                             class="cart_hover_img">
                            <p class="hover_text">Add to Cart</p>
                        </a>
                    </div>
                    <p class="featured_text">Mango People T-shirt</p>
                    <p class="featured_text2">$52.00</p>
                </div>
            </div>
            <div class="featured_link">
                <div class="block__featured">
                    <div class="block__self8"><img src="../assets/img/photo8.jpg" alt="photo___8"
                                                   class="photo___8 featured__photo">
                        <a href="single_page.html" class="cart__hover"> <img src="../assets/img/cart_hover.svg"
                                                                             alt="cart_hover_img"
                                                                             class="cart_hover_img">
                            <p class="hover_text">Add to Cart</p>
                        </a>
                    </div>
                    <p class="featured_text">Mango People T-shirt</p>
                    <p class="featured_text2">$52.00</p>
                </div>
            </div>
        </div>
    </section>
    <div class="clearfix"></div>
    <div class="space center">
        <a href="Product.html" class="button_right">Browse All Product<img src="../assets/img/arrow_right.png"
                                                                           alt="arrow_right"
                                                                           class="arrow_right">
        </a>
    </div>
    <aside class="offer_block">
        <div class="left_block_offer">
            <div class="offer_text_box">
                <h1 class="offer_title">30% <span class="special_color_offer">offer</span></h1>
                <p class="offer_text">for women</p>
            </div>
        </div>
        <div class="right_block_offer">
            <div class="right__subblock_offer">
                <img src="../assets/img/forma_car.png" alt="forma_car" class="forma_car">
                <div class="offer_text_right">
                    <div class="offer_paragraph1">Free Delivery</div>
                    <div class="offer_paragraph2">Worldwide delivery on&nbsp;all. Authorit tively morph
                        next-generation
                        innov tion with extensive models.
                    </div>
                </div>
            </div>
            <div class="right__subblock_offer">
                <img
                        src="../assets/img/forma_sale.png" alt="forma_sale" class="forma_sale">
                <div class="offer_text_right">
                    <div class="offer_paragraph1">Sales&nbsp;&&nbsp;discounts</div>
                    <div class="offer_paragraph2">Worldwide delivery on&nbsp;all. Authorit tively morph
                        next-generation
                        innov tion with extensive models.
                    </div>
                </div>
            </div>
            <div class="right__subblock_offer">
                <img src="../assets/img/forma_king.png" alt="forma_king" class="forma_king">
                <div class="offer_text_right">
                    <div class="offer_paragraph1">Quality&nbsp;assurance</div>
                    <div class="offer_paragraph2">Worldwide delivery on&nbsp;all. Authorit tively morph
                        next-generation
                        innov tion with extensive models.
                    </div>
                </div>
            </div>
        </div>
    </aside>
    </div>
</template>

<script>
    import Products from '@/components/app/Products'
    import Error from '@/components/app/Error'

    export default {
        name: 'mainpage',
        components: {
            Products, Error
        },
        async mounted() {
            await this.$store.dispatch('fetchEnvApi')
        }
    }
</script>
